...
async function render() {
  try {
    ...
    document.querySelector('#sectors').innerHTML = sectors ? `<p>${sectors.rule}</p><p class="weather-metrics">${sectors.reason}</p><div class="weather-indexes">${(sectors.top_boards || []).map(s => `<span>${s.name} ${s.pct_chg}% · 涨停 ${s.limit_up_count} 只</span>`).join('') || '暂无可用板块数据'}</div><p><strong>本日主线：</strong>${(sectors.mainlines || []).map(s => s.name).join('、') || '暂未锁定'}</p>` : '<p class="muted">首次交易日更新后显示。</p>';
